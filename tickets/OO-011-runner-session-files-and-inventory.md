...
- Runtime-validate the snapshot, derive immutable `user_id` from the authenticated runner, upsert missing non-tombstoned five-column catalog rows, and build the control panel's user-scoped session-to-runner route index only in memory. Snapshot absence alone does not delete catalog rows because runner assignment is not persisted; a user-scoped deletion marker prevents upsert and routing.
...
- A snapshot restores a missing five-column catalog row for a valid, non-tombstoned runner-local session under the authenticated runner's owner without importing transcript or runtime state into control persistence.
...
- Snapshot and catalog upsert after runner restart or control crash before catalog insertion, including user-scoped rejection of tombstoned IDs and cross-user project references.
- Control PostgreSQL schema assertion limiting `sessions` to `user_id` plus the four catalog data fields once that table exists.
