...
- Store each provider value in a purpose-classified `encrypted_secrets` row with a UUID primary key, immutable `user_id`, an opaque credential key unique within that user, and the encrypted value.
- Reference each provider secret from a separate `model_provider_credentials` row keyed by immutable user and Pi provider ID. Do not infer provider identity from environment-variable-style secret names, and do not migrate a generic `OPENCODE_API_KEY` secret into a provider credential.
...
- One user's list/read/replace/delete operations cannot observe or mutate another user's row, including when both users configure the same provider.
...
- Two-user tenant-separation coverage for duplicate provider IDs and guessed row identifiers.
