...
- Add deployment, HTTPS, PostgreSQL backup/restore, external `OPENORB_MASTER_KEY` backup/restore, runner session-file backup limitations, runner installation, GitHub token scope, OpenCode Go key, and troubleshooting documentation. The control panel must require no persistent local volume and no Redis/secondary persistence service.
...
- Backup documentation states that both the control PostgreSQL database and externally managed master key are required, that the control panel owns no durable local files, and that runner session-file backup limitations are explicit.
