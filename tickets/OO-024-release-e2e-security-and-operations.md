...
- Backup documentation states that the control PostgreSQL database, externally managed `OPENORB_MASTER_KEY`, and session-cookie signing secret are required to preserve encrypted secrets and valid browser sessions; the control panel owns no durable local files, and runner session-file backup limitations are explicit.
