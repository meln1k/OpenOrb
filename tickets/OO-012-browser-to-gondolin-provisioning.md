...
- Add the exact five-column catalog row (`user_id` plus four catalog data fields) only after runner confirmation; derive ownership from authenticated browser and selected-runner records, and derive the preview by collapsing whitespace and truncating to 200 Unicode code points.
...
- Gateway PostgreSQL `sessions` has exactly `user_id`, `id`, `project_id`, `created_at`, and `initial_prompt_preview`; a composite foreign key ensures the project has the same owner.
- A user cannot provision with another user's project or runner ID; those identifiers are treated as unavailable/not found.
