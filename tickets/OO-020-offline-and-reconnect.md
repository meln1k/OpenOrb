...
- PostgreSQL catalog reconciliation upserts valid five-column rows, derives ownership from the authenticated runner, and filters Workspace-scoped deletion markers. Runner-backed browser routes report offline errors, and an active `EventSource` reconnects after interruption.
...
- Let the runner reconnect transient socket failures with bounded exponential backoff. Admit a replacement connection only after `IdentifyRunner` authentication and one complete, runtime-validated `WatchRunner` snapshot. Subscribe before reading runner files, reconcile the entire snapshot, and atomically install the new connection generation and Workspace-scoped routes only after `snapshot.complete`; a stale generation cannot remove or overwrite replacement routes.
- During snapshot reconciliation, derive tenant ownership from the authenticated runner, upsert missing non-tombstoned five-column catalog rows for valid Workspace-owned projects, and reject catalog conflicts, cross-Workspace references, and Workspace-scoped tombstoned IDs. Request idempotent tombstoned-session cleanup only after provisioning/setup/resume-hook/checkpoint/Git Snapshot/Pi work settles.
...
- A reconnecting runner cannot claim, expose, suppress with a tombstone, or clean up another Workspace's session.
...
- Tombstone/snapshot race, cleanup after active work settles, restored-runner anti-resurrection, and two-Workspace ownership isolation.
