...
- On reconnect, ingest the runner's complete session snapshot, upsert missing non-tombstoned four-field catalog rows, rebuild routes, and restore runner-backed history/event access. Do not remove catalog rows based only on snapshot absence. Reject tombstoned entries and request idempotent runner cleanup after active work settles.
