...
const userMessageEventSchema = object(
  {
    ...
    text: string().refine(
      (value) => value.length > 0 && byteLength(value) <= MAX_SESSION_EVENT_TEXT_BYTES,
      `User messages must contain at most ${MAX_SESSION_EVENT_TEXT_BYTES} UTF-8 bytes.`,
    ),
  },
  { unknownKeys: "error" },
);
...
const assistantCompletedEventSchema = object(
  {
    ...
    text: boundedTextSchema(MAX_SESSION_EVENT_TEXT_BYTES, "Assistant text"),
    thinking: boundedTextSchema(MAX_SESSION_EVENT_TEXT_BYTES, "Assistant thinking"),
    stopReason: union([
...
const toolStartedEventSchema = object(
  {
    ...
    arguments: boundedTextSchema(MAX_SESSION_EVENT_TEXT_BYTES, "Tool arguments"),
  },
  { unknownKeys: "error" },
);
...
const toolCompletedEventSchema = object(
  {
    ...
    toolCallId: eventIdentifierSchema("Tool call identifiers"),
    toolName: eventIdentifierSchema("Tool names"),
    result: boundedTextSchema(MAX_SESSION_EVENT_TEXT_BYTES, "Tool results"),
...
const contextCompactedEventSchema = object(
  {
    ...
    summary: boundedTextSchema(MAX_SESSION_EVENT_TEXT_BYTES, "Compaction summaries"),
...
const provisioningLogEventSchema = object(
  {
    ...
    text: string().refine(
      (value) => value.length > 0 && byteLength(value) <= MAX_SESSION_EVENT_TEXT_BYTES,
      `Provisioning log events must contain at most ${MAX_SESSION_EVENT_TEXT_BYTES} UTF-8 bytes.`,
    ),
  },
  { unknownKeys: "error" },
);
...
const assistantTextDeltaEventSchema = object(
  {
    ...
    delta: boundedTextSchema(MAX_SESSION_EVENT_TEXT_BYTES, "Assistant text deltas"),
  },
  { unknownKeys: "error" },
);
...
const assistantThinkingDeltaEventSchema = object(
  {
    ...
    delta: boundedTextSchema(MAX_SESSION_EVENT_TEXT_BYTES, "Assistant thinking deltas"),
  },
  { unknownKeys: "error" },
);
...
const assistantToolCallDeltaEventSchema = object(
  {
    ...
    contentIndex: nonNegativeIntegerSchema,
    delta: boundedTextSchema(MAX_SESSION_EVENT_TEXT_BYTES, "Assistant tool call deltas"),
  },
  { unknownKeys: "error" },
);
...
const assistantStreamFailedEventSchema = object(
  {
    ...
    errorMessage: optional(boundedTextSchema(MAX_SESSION_EVENT_TEXT_BYTES, "Model errors")),
  },
  { unknownKeys: "error" },
);
...
const toolUpdatedEventSchema = object(
  {
    ...
    toolCallId: eventIdentifierSchema("Tool call identifiers"),
    toolName: eventIdentifierSchema("Tool names"),
    partialResult: boundedTextSchema(MAX_SESSION_EVENT_TEXT_BYTES, "Partial tool results"),
  },
  { unknownKeys: "error" },
);
...
const compactionCompletedEventSchema = object(
  {
    ...
    willRetry: booleanSchema,
    summary: optional(boundedTextSchema(MAX_SESSION_EVENT_TEXT_BYTES, "Compaction summaries")),
    tokensBefore: optional(nonNegativeIntegerSchema),
    estimatedTokensAfter: optional(nonNegativeIntegerSchema),
    errorMessage: optional(boundedTextSchema(MAX_SESSION_EVENT_TEXT_BYTES, "Compaction errors")),
  },
  { unknownKeys: "error" },
);
...
const modelRetryStartedEventSchema = object(
  {
    ...
    errorMessage: boundedTextSchema(MAX_SESSION_EVENT_TEXT_BYTES, "Model retry errors"),
  },
  { unknownKeys: "error" },
);
...
const modelRetryCompletedEventSchema = object(
  {
    ...
    attempt: nonNegativeIntegerSchema,
    finalError: optional(boundedTextSchema(MAX_SESSION_EVENT_TEXT_BYTES, "Model retry errors")),
  },
  { unknownKeys: "error" },
);
...
const summarizationRetryScheduledEventSchema = object(
  {
    ...
    maxAttempts: nonNegativeIntegerSchema,
    delayMs: nonNegativeIntegerSchema,
    errorMessage: boundedTextSchema(MAX_SESSION_EVENT_TEXT_BYTES, "Summarization errors"),
  },
  { unknownKeys: "error" },
);
...
const bashOutputDeltaEventSchema = object(
  {
    ...
    delta: boundedTextSchema(MAX_SESSION_EVENT_TEXT_BYTES, "Bash output deltas"),
  },
  { unknownKeys: "error" },
);
