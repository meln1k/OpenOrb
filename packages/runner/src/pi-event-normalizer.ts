...
export class PiEventNormalizer {
  ...
  handle(event: AgentSessionEvent): void {
    switch (event.type) {
      ...
      case "tool_execution_start":
        this.#enqueue(() =>
          this.#options.publishConversation({
            ...
            arguments: this.#safeText(stringify(event.args), MAX_TOOL_EVENT_TEXT_BYTES),
          })
        );
        return;
      ...
      case "bash_execution_update": {
        const delta = this.#safeText(event.delta, MAX_PROVISIONING_EVENT_TEXT_BYTES);
        if (!delta) return;
        this.#publishLive({
  ...
  #handleAssistantUpdate(event: Extract<AgentSessionEvent, { type: "message_update" }>): void {
    ...
    switch (update.type) {
      ...
      case "text_delta":
      case "thinking_delta": {
        const delta = this.#safeText(update.delta, MAX_PROVISIONING_EVENT_TEXT_BYTES);
        if (!delta) return;
        this.#publishLive(
      ...
      case "toolcall_start":
        ...
        return;
      case "toolcall_delta": {
        const delta = this.#safeText(update.delta, MAX_ACTIVITY_EVENT_TEXT_BYTES);
        if (!delta) return;
        this.#publishLive({
  ...
  #safeError(value: string): string {
    return this.#safeText(value, MAX_ACTIVITY_EVENT_TEXT_BYTES);
  }
  ...
  #queuedMessages(messages: readonly string[]): string[] {
    return messages.slice(0, MAX_QUEUED_SESSION_MESSAGES).map((message) =>
      this.#safeText(message, MAX_QUEUED_MESSAGE_TEXT_BYTES)
    );
  }
